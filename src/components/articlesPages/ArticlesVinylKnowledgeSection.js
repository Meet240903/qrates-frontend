...
const ArticlesVinylKnowledgeSection = () => {
    const truncateTitle = (title, maxLength) => {
        if (!title) return ""; // Return empty string if no title is provided

        return title.length > maxLength ? title.substring(0, maxLength) + "..." : title;
    }
    const selectedArticlesData = articlesPageData?.filter(index=>index?.articleType === 'vinyl knowledge')
    return (
        <>
...
export default ArticlesVinylKnowledgeSection
